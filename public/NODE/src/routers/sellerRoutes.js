...
router.post("/upload-classphoto", upload.single("pic_carousel"), (req, res) => {
  if (req.file && req.file.originalname) {
    switch (req.file.mimetype) {
      ...
      case "image/gif":
        fs.rename(
          req.file.path,
          __dirname + "/../../public/CourseList/" + req.file.originalname,
          (error) => {
            return res.json({
              success: true,
              path: "/CourseList/" + req.file.originalname,
              req: req.file,
            });
          }
        );
